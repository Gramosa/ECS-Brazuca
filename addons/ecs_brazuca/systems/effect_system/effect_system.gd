...
"""O rastreio do tempo deve ser feito para cada efeito, para cada propriedade de cada componente. Ideia: Rergistrar o Timer diretamentev em CalcLink e ele mesmo se remover de seus pais quando o tempo acabar"""
extends BaseSystem
...
## {
##     HealthComponent(id) {
##         "resistance_ratio": [CalcChainA (instance), TimerA (instance)]
##     },
##     DamageComponent(id) {
##         "damage_ratio": [CalcChainB (instance), TimerB (instance)]
## }
##
...
func _modify_property(component: BaseComponent, property: String, effect: EffectData):
	...
	if effect_link == null:
		respective_chain.get_link_by_tag_bfs(effect.chain_tag)\
			.add_numeric_link(effect.effect_value, effect.effect_name)
		...
		effect_timer.timeout.connect(_on_effect_time_timeout, CONNECT_ONE_SHOT)
		effect_timer.start()
...
func apply_effect(source_entity: Node, target_entity: Node, effect_name: String, source_component_name: String = "", target_component_name: String = "") -> void:
	...
	if target_component == null:
		push_warning("Effect Aplication: The effect ({0}) of type ({1}) could not solve, due not found the target compónent".format([effect.effect_name, EffectData.EFFECT_TYPES.keys()[effect.effect_type]]))
		return
